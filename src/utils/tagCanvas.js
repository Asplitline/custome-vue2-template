...
Mproto.mul = function (m) {
  ...
  for (i = 1; i <= 3; ++i) {
    for (j = 1; j <= 3; ++j) {
      if (mmatrix) {
        a.push(this[i][1] * m[1][j] + this[i][2] * m[2][j] + this[i][3] * m[3][j])
      } else {
        a.push(this[i][j] * m)
      }
    }
...
Oproto.DrawColourImage = function (c, x, y, w, h, colour, tag, x1, y1) {
  ...
  } // if using IE and images, give up!
  cc = ocanvas.getContext('2d')
...
Tproto.Measure = function (c, t) {
  ...
  if (t.txtOpt) {
    ...
    if (this.image) {
      tcv.SetImage(this.image, this.iw, this.ih, t.imagePosition, t.imagePadding, t.imageAlign, t.imageVAlign, t.imageScale)
    }
